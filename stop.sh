...
# Funkcja zatrzymująca procesy i usuwająca zasoby IPC
cleanup() {
    ...
    # Usuń wszystkie zbiory semaforów należące do użytkownika
    echo "Usuwam zbiory semaforów..."
    ipcs -s | grep $(whoami) | awk '{print $2}' | while read semid; do
        echo "Usuwam zbiór semaforów SEMID: $semid"
